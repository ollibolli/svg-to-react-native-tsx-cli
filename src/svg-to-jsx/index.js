...
function parseSVG(svg, callback) {
  parseString(svg, {
    ...
    tagNameProcessors: [utils.processTagName],
    validator: cleanupParsedSVGElement
  }, callback);
}

function afterParseSVG({filename}) {
  return (parsedSVG) => {
    utils.forEach(parsedSVG, function(element) {
      ...
      element.attributes = utils.sanitizeAttributes(element.tagName, element.attributes, filename);
      element.children = utils.sanitizeChildren(element.children, filename);
    });
    return parsedSVG;
  }
}
...
module.exports = function svgToJsx(svg, options) {
  options = {...defaults, ...options};
  return q
    .nfcall(parseSVG, svg)
    .then(afterParseSVG({filename: options.filename}))
